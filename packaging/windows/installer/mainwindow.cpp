...
void FindPathsThread::run()
{
	...
	if (m_qtCreatorDirectory != QDir())
		m_qtCreatorDirectory.cdUp();
}
...
void FindPathsThread::find(const QDir &directory)
{
	...
	for (int i = 0; i < list.size(); ++i)
	{
		...
		if (fileInfo.isFile())
		{
			if (fileInfo.fileName() == "qmake.exe")
				m_qtDirectory = fileInfo.absoluteDir();
			else if (fileInfo.fileName() == "qtcreator.exe")
				m_qtCreatorDirectory = fileInfo.absoluteDir();
		}
...
{
	m_maxProgress = countFiles(":/files/");

	installDirectory(":/files/data/plugin", m_qtCreatorDirectory.filePath("bin/plugins/designer"));
	installDirectory(":/files/data/bin", m_qtDirectory.filePath("bin"));
